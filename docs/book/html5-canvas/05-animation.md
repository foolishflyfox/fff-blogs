...
- 不要在 Canvas 中进行带阴影效果的绘制操作
- 不要在播放动画时分配内存

不论是使用 CSS 代码来指定效果，还是直接修改 Canvas 元素属性，阴影与渐变都会降低绘图效率，在移动设备上尤为明显。如果应用程序运行得很慢，那么一定要在启用阴影与渐变效果和不启用效果的情况下分别测试，以确定是否由于启用了阴影与渐变效果而导致程序变慢。
