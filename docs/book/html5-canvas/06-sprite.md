...
行为对象一旦被加入精灵中，它就会随着 update 方法而执行。一般来说，动画循环会反复地调用 `update()` 方法以实现动画效果。实际上，只要某个行为对象被加入到精灵中，那么精灵就会持续地表现出该行为，直到有人将它从精灵的行为数组中移除为止。

然而，有时候你只想让某个行为在一个特定的时间段内执行。比如说，你想让某个正在移动的物体在接受用户输入之后的那一小段时间内，表现出被推动的效果来。

如下所示的应用程序，使用了限时触发行为(timed behavior)对象，此种对象所封装的行为，只会在给定的时间段内表现出来，每当用户点击左箭头或右箭头时，应用程序都将使小球朝着对应的方向移动 200 毫秒。
